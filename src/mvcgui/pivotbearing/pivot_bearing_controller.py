...
from . import pivot_bearing_model
from . import pivot_bearing_view
import numpy
import yaml

class PivotBearingController:
    def __init__(self, index,terminal):
        self.view = pivot_bearing_view.PivotBearingView()
        self.model = pivot_bearing_model.PivotBearingModel()
        ...
        for name in self.categories:
            # RevolutionCtrl and CurrenCtrl currently not supported but ready to be supported. Delete if statement below to activate in ui
            # delete the same if statement in self.build_commands() function
            if name == "RevolutionCtrl" or name == "CurrentCtrl":
                continue
            for key in self.categories[name]:
                ...
                if self.config_data[name][key]["ValidCallback"] == "list":
                    self.model.add_data_discrete(key, self.config_data[name][key]["Range"])

                elif self.config_data[name][key]["ValidCallback"] == "string" or self.config_data[name][key]["ValidCallback"] == "range":
                    min = self.config_data[name][key]["Range"]["min"]
                    if min == "MIN" and self.config_data[name][key]["Type"] != "float":
                        min = self.data_types[self.config_data[name][key]["Type"]]["min"]
                        print(f"Name: {name} Key: {key} min: {min}")

                    elif min == "MIN" and self.config_data[name][key]["Type"] == "float":
                        dtype = numpy.float32
                        min = numpy.finfo(dtype).min
                        print(f"Name: {name} Key: {key} min: {min}")

                    max = self.config_data[name][key]["Range"]["max"]
                    if max == "MAX" and self.config_data[name][key]["Type"] != "float":
                        max = self.data_types[self.config_data[name][key]["Type"]]["max"]
                        print(f"Name: {name} Key: {key} max: {max}")

                    elif max == "MAX" and self.config_data[name][key]["Type"] == "float":
                        dtype = numpy.float32
            ...
            self.view.inner_views[name]["button_all"]
    ...
    def build_commands(self):
        ...
        for name in self.categories:
            if name == "RevolutionCtrl" or name == "CurrentCtrl":
                continue
            for key in self.categories[name]:
                com[key] = self.global_param[name].replace("<<index>>", str(self.index)) + "." + key
